...
async function generateImageGrid(
  ...
  for (let i = 0; i < Math.min(albums.length, cols * rows); i++) {
    ...
    try {
      ...
      if (!imageResponse.ok)
        throw new Error(
          `Failed to fetch image: ${imageResponse.statusText} for ${album.name}`
        );
      const imageBuffer = await imageResponse.buffer();

      let tileImage = sharp(imageBuffer).resize(TILE_SIZE, TILE_SIZE, {
        fit: 'cover',
      });
      ...
      if (showInfo) {
        ...
        textElements.push(
          `<text x="${PADDING}" y="${textYBase + 12}" font-family="sans-serif" font-size="12px" fill="white" font-weight="bold">${escapeXml(album.name)}</text>`
        );
        textElements.push(
          `<text x="${PADDING}" y="${textYBase + 24}" font-family="sans-serif" font-size="10px" fill="white">${escapeXml(album.artist.name)}</text>`
        );
        if (showPlaycount && album.playcount) {
          textElements.push(
            `<text x="${PADDING}" y="${textYBase + 36}" font-family="sans-serif" font-size="10px" fill="white">${album.playcount} plays</text>`
          );
        }
        ...
        tileImage = tileImage.composite([
          { input: Buffer.from(svgTextOverlay), gravity: 'southwest' },
        ]);
      }
      ...
      console.error(
        `Error processing image for album ${album.name}: ${message}`
      );
      const errorSvg = `
        <svg width="${TILE_SIZE}" height="${TILE_SIZE}">
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (
      ...
      topAlbumsData.topalbums.album.length === 0
    ) {
      return NextResponse.json(
        { error: 'No albums found for this user or parameters.' },
        { status: 404 }
      );
    }
    ...
    const imageBuffer = await generateImageGrid(
    ...
    if (imageBuffer) {
      ...
      return NextResponse.json(
        { error: 'Image generation failed or no albums to process.' },
        { status: 500 }
      );
    }
  } catch (error) {
    ...
    if (
      errorMessage.toLowerCase().includes("cannot find module 'sharp'") ||
      errorMessage.toLowerCase().includes('sharp')
    ) {
      console.error(
        'Sharp library might be missing or not installed correctly.'
      );
    }
    return NextResponse.json(
